...
class ImageHandler:
    ...
    def check_point_inside_rects(self, point, rects):
    ...
    def find_image_location(self, image, parent_image):
        if parent_image is None or image is None:
            return None
        # Find the location of the image in the parent image
        if self.debug_mode:
            cv2.imshow('shapes', np.array(parent_image))
            cv2.waitKey(self.debug_image_show_milliseconds)

        gray = cv2.cvtColor(np.array(parent_image), cv2.COLOR_BGR2GRAY)
    ...
    def find_all_image_locations(self, image, parent_image):
        if parent_image is None or image is None:
            return None
        # Find the location of the image in the parent image
        if self.debug_mode:
    ...
    def find_text_in_array(self, text, arrays, windows):
        ...
        """
        for r in arrays:
            if text in r[1] and self.check_point_inside_rects(r[0][0], windows):
                return r[0][0]
        return None
