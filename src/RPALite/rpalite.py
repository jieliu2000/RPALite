...
class RPALite:
    ...
    def __init__(self, step_pause_interval=3, debug_mode=False, languages=['en']):
        ...
        step_pause_interval : int
            The default pause interval in seconds after each step. The default value of this attribute is 3 seconds.    
    ...
    def find_text_positions(self, text, filter_args_in_parent=None, parent_control=None, img=None, exact_match=False):
        ...
        exact_match : bool
            Whether to perform an exact match. If set to True, the function will only return the location if the text is exactly the same as the text parameter. Default is False.
        ...
        if not locations:
            return None
        if exact_match and (len(locations[0]) < 2 or not locations[0][1] or (not (locations[0][1] in text) and not (text in locations[0][1]))):
            return None
    ...
    def locate(self, location_description, parent_image=None, app=None):
        ...
        if isinstance(location_description, str):
            ...
            if location_description.startswith('automateId:'):
                ...
                logger.debug(f'Clicking by automate id: {automate_id}')
                if app is None:
    ...
    def click_by_text_inside_window(self, text, window_title, button='left', double_click=False):
        """Click the position of a string on screen. """
        ...
        if not rects:
            ...
            locations = self.find_text_positions(text, parent_control=rects, img=img)
    ...
    def input_text(self, text, seconds=0):
        ...
        text : str
            Text to type
        seconds : int
            The time to sleep after typing the text
        """
    ...
    def get_text_field_value(self, field_name):
        ...
        if location is None:
            logger.error(f'Cannot find field: {field_name}')
            return ''
    ...
    def enter_in_field(self, field_name, text):
        """
        Enters text in a field identified by field_name parameter.
        This function is an automation method and will not return any value.
